          ...
          <tbody>
            {leaves.map((l) => (
              <tr key={l.id} className="border-b last:border-0 hover:bg-gray-50">
                <td className="px-4 py-3 font-medium">{l.employee_id}</td>
                <td className="px-4 py-3">
                  <span className="bg-gray-100 px-2 py-0.5 rounded text-xs">{TYPE_THAI[l.leave_type] || l.leave_type}</span>
                </td>
                <td className="px-4 py-3 text-gray-600">{l.start_date} — {l.end_date}</td>
                <td className="px-4 py-3">{l.days_count} วัน</td>
